...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (1633987,1635142))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (1633987,1635142))
) I
) C;
...
(
  select m.person_id,m.measurement_id,m.measurement_concept_id,m.visit_occurrence_id,m.value_as_number,m.range_high,m.range_low,m.measurement_date as start_date, DATEADD(day,1,m.measurement_date) as end_date , row_number() over (PARTITION BY m.person_id ORDER BY m.measurement_date, m.measurement_id) as ordinal
  FROM @cdm_database_schema.MEASUREMENT m
JOIN isap3ti5Codesets cs on (m.measurement_concept_id = cs.concept_id and cs.codeset_id = 5)
) C
WHERE C.ordinal = 1
-- End Measurement Criteria
  ) E
...
UNION ALL
select inclusion_rule_id, person_id, event_id from isap3ti5Inclusion_3) I;

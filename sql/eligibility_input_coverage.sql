...
-- both within +/- @lab_index_window_days of the Target 1A index date.
...
--                   @raw_lab_results_table, @cohort1_id, @lab_index_window_days
WITH t1a AS (
...
passed AS (
  SELECT lab.cohort_definition_id AS test_id,
    ...
    JOIN @work_database_schema.@lab_cohort_table lab
     ...
     AND lab.cohort_start_date BETWEEN DATEADD(day, -@lab_index_window_days, t1a.cohort_start_date)
                                   AND DATEADD(day,  @lab_index_window_days, t1a.cohort_start_date)
...
tested AS (
  SELECT raw.test_id AS test_id,
    ...
    JOIN @work_database_schema.@raw_lab_results_table raw
     ...
     AND raw.measurement_date BETWEEN DATEADD(day, -@lab_index_window_days, t1a.cohort_start_date)
                                  AND DATEADD(day,  @lab_index_window_days, t1a.cohort_start_date)

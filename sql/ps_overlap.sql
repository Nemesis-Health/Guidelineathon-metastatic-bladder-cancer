...
-- ECOG record in that stratum's id-set within +/- @window days of their 1A
-- index. Strata OVERLAP by design (e.g. PS 0-2 includes PS1/PS2).
--
-- SqlRender parameters:
--   @work_database_schema @cohort_table @lab_cohort_table @cohort1_id @window
-- =============================================================================
 ...
 WHERE tc.cohort_definition_id = @cohort1_id
   AND EXISTS (SELECT 1 FROM @work_database_schema.@lab_cohort_table l
                WHERE l.subject_id = tc.subject_id AND l.cohort_definition_id IN (24)
                  AND l.cohort_start_date BETWEEN DATEADD(day, -@window, tc.cohort_start_date)
                                              AND DATEADD(day,  @window, tc.cohort_start_date))
UNION ALL
